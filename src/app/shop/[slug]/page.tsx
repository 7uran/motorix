...
export default function Page() {
    ...
    useEffect(() => {
        if (id) {
            const fetchProduct = async () => {
                try {
                    setLoading(true);
                    const data = await getData(`http://localhost:3001/api/v1/shopCards/${id}`);
                    if (data) {
                        setProduct(data);
                    }
                } catch (err) {
    ...
    const averageRating = product.comments.length > 0
        ? (product.comments.reduce((acc, comment) => acc + comment.rating, 0) / product.comments.length).toFixed(2)
        : '0.00';
        ...
        <div className="min-h-screen bg-white">
            <div className="max-w-[1291px] mx-auto flex py-28 gap-16">
                ...
                <div className="w-[507px] flex flex-col gap-3">
                    ...
                    <Accordion title="Delivery Info" content={
                        <div className="flex flex-col gap-6">
                            <div className="flex flex-col gap-2">
                                ...
                                <p>{product.comments.length} review{product.comments.length > 1 ? 's' : ''}</p>
                                        ...
                                        <div className="flex items-center gap-6 w-full">
                                            ...
                                            <div className="w-full bg-gray-200 h-1.5">
                                                <div
                                                    ...
                                                    style={{
                                                        width: `${(product.comments.filter(comment => comment.rating === parseInt(rating)).length / product.comments.length) * 100}%`,
                                                    }}
                                                />
                                            </div>
                                        </div>
                                        <p className="text-stone-500 text-lg">
                                            {Math.round((product.comments.filter(comment => comment.rating === parseInt(rating)).length / product.comments.length) * 100)}%
                                        </p>
                                    </div>
                                ))}
                            </div>
                            <div className="flex flex-col gap-4">
                                <span className="text-xl text-black font-medium">{product.comments.length} review{product.comments.length > 1 ? 's' : ''} for {product.name}</span>
                                {product.comments.map((comment, index) => (
                                    <div key={index} className="flex gap-5">
                                        <div >
                                            <Image className="rounded-full" alt="" height={60} width={60} src={"https://secure.gravatar.com/avatar/b79b28b41492262d0932be1d95b4bb2d?s=90&d=mm&r=g"} />
                                        </div>

                                        <div className="flex flex-col gap-2">
                                            <p className="flex gap-1 text-black font-medium items-center">
                                                {comment.publisher} - <span className="font-thin text-sm text-stone-500">{'Dec 7, 2023'}</span>
                                            </p>
                                ...
                                <div className="flex flex-col gap-6">
                                    ...
                                    <button
                                        className="bg-main text-white px-8 py-3 mt-4"
